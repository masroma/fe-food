<template>
    <div class="bg-white shadow-lg p-5 py-10">
     <div class="flex justify-between">
      ...
      <nuxt-link :to="{name: 'admin-products-create'}" class="bg-blue-500 py-2 text-white rounded cursor-pointer px-5 flex gap-2">
      ...
      <div class="container mx-auto my-5" v-if="!showLoader">
        ...
        <table class="min-w-full border-collapse border border-gray-300">
          ...
          <tbody>
            ...
            <tr class="bg-white" v-for="row in products.data" :key="row.id">
              ...
              <td class="border p-2">
               <div class="flex gap-2">
                <nuxt-link :to="{name: 'admin-products-edit-id', params: {id: row.id}}" class="bg-green-500 py-2 text-white rounded cursor-pointer px-3 flex gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        <div class="flex justify-between mt-10">
          <p>Menampilkan {{products.to}} dari {{products.total}}</p>
          <ul class="flex space-x-2">
              <!-- Tombol "Previous" -->
              <li >
                  <a v-if="products.current_page != 1" @click="changePage(products.current_page - 1)" class="px-4 py-2 bg-blue-500 text-white rounded-l-md cursor-pointer">Previous</a>
  
                  <a v-if="products.current_page === 1"  class="px-4 py-2 bg-blue-300 bg-blue-300 text-white rounded-l-md">Previous</a>
              </li>
              ...
              <!-- Tombol "Next" -->
              <li>
                  <a @click="changePage(products.current_page + 1)" v-if="products.current_page != products.last_page" class="px-4 py-2 bg-blue-500 cursor-pointer text-white rounded-r-md">Next</a>
  
                  <a  v-if="products.current_page === products.last_page" class="px-4 py-2 bg-blue-300 text-white rounded-r-md">Next</a>
              </li>
          </ul>
      </div>
        ...
        this.$swal
          ...
          .then((result) => {
            if (result.isConfirmed) {
              ...
              this.$store
                .dispatch("admin/product/destroy", id)
                .then(() => {
                  //feresh data
                  // this.$nuxt.refresh()
