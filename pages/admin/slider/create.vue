<template>
  <div class="bg-white shadow-lg p-5 py-10">
    <div class="flex justify-between">
      <p class="text-lg font-semibold">Tambah Sliders</p>
    </div>
      ...
      <form @submit.prevent="storeSlider">
        ...
        <div class="flex flex-col mb-4">
          ...
          <input
            ...
            name="name"
            v-model="slider.link"
            :class="['shadow appearance-none border', validation.link ? 'border-red-500' : 'border', 'rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline focus:shadow-outline']"
            placeholder="Nama kategori"
          />
          <span class="text-red-700" v-if="validation.link">
            {{ validation.link[0] }}</span
          >
        </div>
